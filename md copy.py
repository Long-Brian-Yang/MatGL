...
class MDSystem:
    ...
    def __init__(
        ...
        self.potential = matgl.load_model(model_path)

        self.calculator = PESCalculator(self.potential)
            
        self.logger.info(f"Loaded potential model: {model_path}")
...
#         'random_state': 42
#     }
#     # Define path to fine-tuned model
#     model_path = os.path.join(paths['output_dir'], 'checkpoints')
    
#     # Initialize MD system
#     md_system = MDSystem(
#         config=config,
#         model_path=model_path, # Path to fine-tuned model
#         time_step=1.0,
#         friction=0.02,
#         total_steps=20000,
#         output_interval=100
...
def main():
    ...
    # Define path to fine-tuned model
    model_path = os.path.join(paths['output_dir'], 'checkpoints', 'model.pth')
